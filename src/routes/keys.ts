...
router.get("/keys", authMiddleware, (_req, res) => {
...
router.post("/keys", authMiddleware, (req, res) => {
  ...
  const key = dbOps.createKey(type, Number(duration_days), OWNER_ID);
  res.status(201).json(key);
});

router.post("/keys/batch", authMiddleware, (req, res) => {
  ...
  const keys = dbOps.createKeys(type, Number(duration_days), OWNER_ID, qty);
...
router.post("/keys/:id/reset-ip", authMiddleware, (req, res) => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  const keys = dbOps.getAllKeys();
  const key = keys.find((k) => k.id === id);
  if (!key) { res.status(404).json({ error: "Key not found" }); return; }
  const result = dbOps.resetKeyIp(key.key);
  if (!result.ok) {
